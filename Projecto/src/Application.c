...
int initAppLayer(char * port, Mode connectionMode, int baudRate, int messageMaxSize, int retries, int timeout, char* fileName){
  ...
  if(!startLinkLayer(appLayer->fd, port, connectionMode, baudRate, messageMaxSize, timeout, retries)){
...
int send(){
  ...
  fseek(file, 0, SEEK_END); // file points to the end of the file
  unsigned int fileSize = ftell(file); // get the file size
  fseek(file, 0, SEEK_SET); // return pointer to the start of the file
